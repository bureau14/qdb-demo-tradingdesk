...
struct Renderable {
  virtual Image render(size_t w)const = 0;
};
...
template<class R>
auto getImage(size_t w, R const& r) -> decltype(isImage(r.render(w))) {
  return r.render(w);
}

template<class R>
auto getImage(size_t w, R const& r) -> decltype(isImage(r(w))) {
  return r(w);
}

template<class R>
auto getImage(size_t w, R const& r) -> decltype(isImage(r)) {
  return w;
}

template<class R>
auto getImage(size_t w, R const& r) -> decltype(Text{{}, toString(r)}.render(w)) {
...
struct Component : Renderable {
  ...
  virtual Image render(size_t w)const override{
    return ptr->render(w);
  }
