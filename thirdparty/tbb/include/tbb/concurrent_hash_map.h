...
public:
    ...
    class const_accessor : public node::scoped_t /*which derived from no_copy*/ {
        friend class concurrent_hash_map<Key,T,HashCompare,Allocator>;
        friend class accessor;

	public:
		using write_accessor_type = accessor;
    ...
    public:
        ...
        void swap(Accessor & acc) noexcept
        {
            std::swap(node::scoped_t::mutex, acc.mutex);
            node::scoped_t::is_writer = acc.is_writer();
            std::swap(my_node, acc.my_node);
            std::swap(my_hash, acc.my_hash);
        }
